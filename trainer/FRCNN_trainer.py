...
class COWCFRCNNTrainer:
    ...
    def data_loaders(self):
        ...
        dataset = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_LR_train"],
            image_height=64,
            image_width=64,
            transforms=self.get_transform(train=True),
        )
        dataset_test = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_Valid"],
            image_height=64,
            image_width=64,
            transforms=self.get_transform(train=False),
        )
        dataset_test_SR = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_SR"],
            transforms=self.get_transform(train=False),
        )
        dataset_test_SR_combined = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_SR_combined"],
            transforms=self.get_transform(train=False),
        )
        dataset_test_E_SR_1 = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_E_SR_1"],
            transforms=self.get_transform(train=False),
        )
        dataset_test_E_SR_2 = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_E_SR_2"],
            transforms=self.get_transform(train=False),
        )
        dataset_test_E_SR_3 = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_E_SR_3"],
            transforms=self.get_transform(train=False),
        )
        dataset_test_F_SR = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_F_SR"],
            transforms=self.get_transform(train=False),
        )
        dataset_test_Bic = COWCFRCNNDataset(
            root=self.config["path"]["data_dir_Bic"],
            transforms=self.get_transform(train=False),
        )
        ...
        return (
            ...
            data_loader_test,
            data_loader_test_SR,
            data_loader_test_SR_combined,
            data_loader_test_E_SR_1,
            data_loader_test_E_SR_2,
            data_loader_test_E_SR_3,
            data_loader_test_F_SR,
            data_loader_test_Bic,
        )
    ...
    def draw_detection_boxes(self, new_class_conf_box, file_path):
        source_image_path = os.path.join(
            self.config["path"]["data_dir_Bic_valid"],
    ...
    def get_prediction(self, model, images, annotation_path, threshold=0.5):
        ...
        file_path = os.path.join(
            self.config["path"]["Test_Result_LR_LR_COWC"],
            os.path.basename(annotation_path),
        )
        # print(file_path)
        pred_class = [
            i for i in list(outputs[0]["labels"].detach().cpu().numpy())
        ...
        for i in range(len(text_boxes)):
            ...
            new_class_conf_box.append(
                [
                    pred_class[i],
                    pred_score[i],
                    int(text_boxes[i][0] * 4),
                    int(text_boxes[i][1] * 4),
                    int(text_boxes[i][2] * 4),
                    int(text_boxes[i][3] * 4),
                ]
            )
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            train_one_epoch(
                model, optimizer, data_loader, self.device, epoch, print_freq=10
            )
