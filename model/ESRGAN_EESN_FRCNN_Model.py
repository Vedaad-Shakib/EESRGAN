...
class ESRGAN_EESN_FRCNN_Model(BaseModel):
    def __init__(self, config, device):
        ...
        # descriminator
        self.netD = model.Discriminator_VGG_128(
            in_nc=self.configD["in_nc"], nf=self.configD["nf"]
        )
        ...
        if self.cri_fea:  # load VGG perceptual loss
            self.netF = model.VGGFeatureExtractor(
                feature_layer=34, use_input_norm=True, device=self.device
            )
        ...
        self.D_update_ratio = (
            self.configT["D_update_ratio"] if self.configT["D_update_ratio"] else 1
        )
        self.D_init_iters = (
            self.configT["D_init_iters"] if self.configT["D_init_iters"] else 0
        )
        ...
        self.optimizer_FRCNN = torch.optim.SGD(
            FRCNN_params, lr=0.005, momentum=0.9, weight_decay=0.0005
        )
    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_fea:  # feature loss
                real_fea = self.netF(
                    self.var_H
                ).detach()  # don't want to backpropagate this, need proper explanation
    ...
    def test(self, valid_data_loader, train=True, testResult=False, inference=False, tb_logger=None, current_step=0):
        self.netG.eval()
        self.netFRCNN.eval()
        ...
        if testResult == False:
            with torch.no_grad():
                (
                    self.fake_H,
                    self.final_SR,
                    self.x_learned_lap_fake,
                    self.x_lap,
                ) = self.netG(self.var_L)
                self.x_lap_HR = kornia.laplacian(self.var_H, 3)
        if train == True: # this trains it
            evaluate(self.netG, self.netFRCNN, self.targets, self.device, tb_logger=tb_logger, current_step=current_step)
    ...
    def print_network(self):
        ...
        if self.cri_fea:  # F, Perceptual Network
            ...
            if isinstance(self.netF, nn.DataParallel) or isinstance(
                ...
                net_struc_str = "{} - {}".format(
                    self.netF.__class__.__name__, self.netF.module.__class__.__name__
                )
            else:
    ...
    def load(self):
        ...
        logger.info(f"load_path_G: {load_path_G}")
        if load_path_G:
            logger.info("Loading model for G [{:s}] ...".format(load_path_G))
            self.load_network(
                load_path_G, self.netG, self.config["path"]["strict_load"]
            )
        load_path_D = self.config["path"]["pretrain_model_D"]
        if load_path_D:
            logger.info("Loading model for D [{:s}] ...".format(load_path_D))
            self.load_network(
                load_path_D, self.netD, self.config["path"]["strict_load"]
            )
        load_path_FRCNN = self.config["path"]["pretrain_model_FRCNN"]
        if load_path_FRCNN:
            logger.info("Loading model for D [{:s}] ...".format(load_path_FRCNN))
            self.load_network(
                load_path_FRCNN, self.netFRCNN, self.config["path"]["strict_load"]
            )
